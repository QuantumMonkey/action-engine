...
| REQ-15 | chapter B, ADR-0002 | TASK-B6 | DONE 2026-09-24: verified against a real Postgres 16 service in CI (run 35989837284, acceptance 9 passed); runner is dialect-aware and idempotent. Amended 2026-09-25: the image push step was conditional on master and had never run, so the first master run failed on an uppercase registry name -- fixed and reverified on a real master run. See DEVIATIONS.md; a conditional CI step is not evidence until its condition is true |
| REQ-16 | chapter B | TASK-B7 | PARTIAL 2026-09-25: half done. The image half is finished -- chapter B merged to master (1eb6eeb), CI green on both gates (suites, then image built and smoke-tested: healthz, openapi, 401 anonymous, 503 unverifiable token), and `ghcr.io/quantummonkey/action-engine:latest` is published and pullable with no credentials (tags list fetched anonymously on 2026-09-25). The URL half is not: ADR-0003 chose Azure Container Apps plus Neon, and the deploy runs on the operator's own accounts. Not claimed as done until a stranger's curl answers |
